...
train_ratings_combined = torch.cat([train_rating, train_negative_ratings]).unsqueeze(1)
# 生成验证集负样本，并合并数据
...
valid_ratings_combined = torch.cat([valid_rating, valid_negative_ratings]).unsqueeze(1)
# 生成测试集负样本，并合并数据
...
test_ratings_combined = torch.cat([test_rating, test_negative_ratings]).unsqueeze(1)
