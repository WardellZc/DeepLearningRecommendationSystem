...
train_negative = train_sampler.negative_sampling2(data.num_users, data.num_items, excluded_pairs, 60)  # 生成用户、项目、评分数据
...
valid_negative = valid_sampler.negative_sampling2(data.num_users, data.num_items, excluded_pairs, 20)
...
test_negative = test_sampler.negative_sampling2(data.num_users, data.num_items, excluded_pairs, 20)
...
train_data = torch.tensor(train_feature.iloc[:, 3:].values, dtype=torch.float32)  # 去除user_di、item_id、rating
train_rating = torch.tensor(train_feature.iloc[:, 2].values, dtype=torch.float32).unsqueeze(1)  # 评分数据，用于计算损失
valid_data = torch.tensor(valid_feature.iloc[:, 3:].values, dtype=torch.float32)
valid_rating = torch.tensor(valid_feature.iloc[:, 2].values, dtype=torch.float32).unsqueeze(1)
test_data = torch.tensor(test_feature.iloc[:, 3:].values, dtype=torch.float32)
test_rating = torch.tensor(test_feature.iloc[:, 2].values, dtype=torch.float32).unsqueeze(1)

# 定义模型
model = FFM(train_data.size(1), 32)
